...
router.get("/history", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (type === "EARNED" || type === "SPENT") where.type = type;
...
// Analytics endpoints
router.get("/analytics/credits", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "User not authenticated" });
    const prisma = getPrisma();
    ...
    for (const e of entries) {
      ...
      if (!buckets[key]) buckets[key] = { earned: 0, spent: 0 };
      if (e.type === "EARNED") buckets[key].earned += Number(e.hours);
      else buckets[key].spent += Number(e.hours);
    }
...
router.get("/analytics/collaborators", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ error: "User not authenticated" });
    const prisma = getPrisma();
